...
def my_open_write(file, force):
    if not force and os.path.exists(file):
        judge = input("このファイルを上書きしてもいいですか? %s [y/n]" % file)
        if judge == 'n':
            return None
...
class TestBench:
    ...
    def __init__(self, file_list, input='', output='', topmodule='', path='', time=0, force=False):
        for file in file_list:
            check_veri(file)
    ...
    def output_file(self):
        f = my_open_write(self.dest_file, self.force)
        if f is None:
            return
