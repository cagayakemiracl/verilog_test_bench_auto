...
class Test_bench:
    ...
    def analysis(self):
        target = False
        with open(self.dest_file, 'w') as f1, open(self.sorce_file, 'r') as f2:
            f1.write("""/*
...
""")
            for line in f2:
                if "input" in line and target:
                    bit_num = 0
                    line = print_type("input", line)
                    f1.write("\treg  %s\n" % line)
                    attr = my_split(",|;|\s", line)
                    if re.match("^\[", attr[0]):
                    ...
                    print_type("endmodule", line)
                    break
                elif "module" in line:
                    line = print_type("module", line)
                    tmp = my_split("\s", line)
                    name = tmp[0]
                    print(name)
                    if self.base == name:
